...
import { FOLLOWER_USER_URL, FOLLOWING_URL, PLAYLIST_COUNT_URL, TOP_ARTISTS_URL, TOP_TRACKS_URL, RECENT_URL, AUTH_CODE_KEY } from "../utils/constants";

const infoSlice = createSlice({
  name: "infoApi",
  initialState: { loading: false, username: false, profileStatusArr: false, followers: 0, userAvatarSrc: false, following: 0, playlists: 0, playlistArr: false, topArtistsList: false, topTracksList: false, recentPlayedList: false },
  reducers: {},
      ...
      .addCase(getDataThunk.fulfilled, (state, action) => {
        ...
        if (status === 200) {
          ...
          if (url === TOP_ARTISTS_URL) {
            state.topArtistsList = data.items.map((ele) => {
              return { imageSrc: ele.images[0].url, artistName: ele.name };
            });
          }
          if (url === TOP_TRACKS_URL) {
            state.topTracksList = data.items.map((ele) => {
